...
async function moveJurnalToTrash(id) {
...
function filterBankRecAnalysisRows(tableId, query, kind) {
  ...
  root.querySelectorAll('.bankrec-group').forEach(function(group) {
    var groupVisible = 0;
    group.querySelectorAll('tbody tr[data-kind]').forEach(function(r) {
      if (r.style.display !== 'none') groupVisible++;
    });
    group.style.display = groupVisible ? '' : 'none';
    var badge = group.querySelector('.bankrec-group-count');
    if (badge) badge.textContent = groupVisible + ' item';
  });
  var empty = document.getElementById('bankrec-analysis-empty');
  if (empty) empty.style.display = visibleCount ? 'none' : 'block';
}

function setBankRecGroupState(containerId, shouldOpen) {
...
async function integrasiPermohonanDanaMasukKeJurnal() {
  ...
  // Filter: Approved Final & jatuh tempo sudah tiba & (belum punya jurnal ATAU jurnalnya sudah terhapus)
  var todayStr = today();
  var pdBelumJurnal = allPD.filter(function(p) {
    if (p.status !== 'Approved Final') return false;
    if (!p.jatuhTempo || p.jatuhTempo > todayStr) return false; // jatuh tempo belum tiba, jangan dijurnal
  ...
  var dmBelumJurnal = allDM.filter(function(d) {
    ...
    var jt = d.tanggal || '';
    if (!jt || jt > todayStr) return false; // jatuh tempo belum tiba, jangan dijurnal
...
async function autoJurnalJatuhTempo() {
  ...
  try {
    ...
    // Permohonan: Approved Final, jatuh tempo sudah tiba, belum dijurnal
    var pdReady = allPD.filter(function(p) {
      if (p.status !== STATUS.APPROVED) return false;
      if (!p.jatuhTempo || p.jatuhTempo > todayStr) return false;
      if (!p.jurnalId) return true;
      if (jurnalIds.indexOf(p.jurnalId) === -1) return true;
      return false;
    });

    // Dana Masuk: Approved Final, tanggal (jatuh tempo) sudah tiba, belum dijurnal
    var dmReady = allDM.filter(function(d) {
      if (d.status !== STATUS.APPROVED) return false;
      var jt = d.tanggal || '';
...
async function renderForecastBayar() {
  ...
  const pdForecast = allPD.filter(function(x){
    ...
    // Future items ALWAYS shown in forecast regardless of jurnalId or status
    if (x.jatuhTempo && x.jatuhTempo > todayStr) return true;
    // Past-due or today: exclude only if properly journalized (has valid jurnalId AND Approved Final)
    if (x.jatuhTempo && x.jatuhTempo <= todayStr) {
      if (x.jurnalId && jurnalIds.indexOf(x.jurnalId) !== -1 && x.status === STATUS.APPROVED) return false;
      return true;
    }
    // No jatuhTempo: exclude if properly journalized
    if (!x.jatuhTempo) {
      if (x.jurnalId && jurnalIds.indexOf(x.jurnalId) !== -1 && x.status === STATUS.APPROVED) return false;
      return true;
    }
...
async function renderForecastTerima() {
  ...
  const dmForecast = allDM.filter(function(x){
    ...
    // No tanggal: exclude if properly journalized
    if (!jt) {
      if (x.jurnalId && jurnalIds.indexOf(x.jurnalId) !== -1 && x.status === STATUS.APPROVED) return false;
      return true;
    }
...
function renderForecastWithChart(list, title, chartId, saldoData) {
  ...
  return '<div class="page-title" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">' + (chartId==='terima'?'📈':'📉') + ' ' + title + navBtn + '</div>'
    ...
    + (list.length ? '<div class="table-wrap"><table><thead><tr><th>Nama</th><th>Referensi</th><th>Jatuh Tempo</th><th>Sisa</th><th>Sumber</th><th>Status</th><th>Aksi</th></tr></thead><tbody>' + rows + '</tbody></table></div>' : '<div class="empty-state"><span class="icon">📊</span>Tidak ada outstanding</div>')
    + '</div>';
}
...
function buildLineChart(datasets, labels, chartId) {
  var svgWidth = 700, svgHeight = 240;
...
async function analisaSelisihSaldo(skipPrompt) {
  ...
  // 6. Hitung total debit & kredit ke kas mandiri (exclude orphan jurnals)
  var totalDebitKas = 0, totalKreditKas = 0, countTrx = 0;
  var orphanDebitKas = 0, orphanKreditKas = 0;
  jurnal.forEach(function(j) {
    var isOrphan = !!orphanIds[j.id];
    (j.lines || []).forEach(function(l) {
      if (l.akun === kasAkun) {
        var deb = parseFloat(l.debit) || 0;
        var kre = parseFloat(l.kredit) || 0;
        if (isOrphan) {
          orphanDebitKas += deb;
          orphanKreditKas += kre;
        } else {
          totalDebitKas += deb;
          totalKreditKas += kre;
          if (deb > 0 || kre > 0) countTrx++;
        }
      }
    });
  });
  var orphanImpact = orphanDebitKas - orphanKreditKas;
  var saldoSistemAdjusted = saldoSistem - orphanImpact;
  var selisihAdjusted = saldoSistemAdjusted - saldoReal;
  ...
  if (orphanImpact !== 0) {
    html += '<tr style="background:#fff8e1"><td style="padding:8px">👻 Dampak Jurnal Yatim (Orphan)</td><td class="text-right fw-bold text-red">' + fmtRp(Math.abs(orphanImpact)) + (orphanImpact > 0 ? ' (inflate saldo)' : ' (deflate saldo)') + '</td></tr>'
    + '<tr style="background:#e8f5e9"><td style="padding:8px"><b>Saldo Adjusted (tanpa Orphan)</b></td><td class="text-right fw-bold">' + fmtRp(saldoSistemAdjusted) + '</td></tr>'
    + '<tr style="background:' + (selisihAdjusted===0?'#e8f5e9':'#ffebee') + '"><td style="padding:8px"><b>SELISIH ADJUSTED</b></td><td class="text-right fw-bold ' + (selisihAdjusted===0?'text-green':'text-red') + '">' + fmtRp(Math.abs(selisihAdjusted)) + (selisihAdjusted > 0 ? ' (Sistem lebih besar)' : selisihAdjusted < 0 ? ' (Bank lebih besar)' : ' ✅ Match') + '</td></tr>';
  }
  html += '<tr><td style="padding:8px;color:#888">Saldo Awal ' + tahun + '</td><td class="text-right">' + fmtRp(saldoAwalMandiri) + '</td></tr>'
  ...
  // Summary orphan jurnal impact
  if (orphanImpact !== 0) {
    html += '<div style="margin:12px 0;padding:10px 14px;background:#fce4ec;border-radius:8px;border-left:4px solid #e91e63;font-size:0.82rem">'
      + '<b>👻 Dampak Jurnal Yatim (Orphan) ke saldo Mandiri:</b><br>'
      + '• Orphan Debit: <b class="text-green">' + fmtRp(orphanDebitKas) + '</b> | Orphan Kredit: <b class="text-red">' + fmtRp(orphanKreditKas) + '</b><br>'
      + '• Total dampak orphan: <b class="text-red">' + fmtRp(Math.abs(orphanImpact)) + '</b> (' + (orphanImpact > 0 ? 'menambah saldo sistem' : 'mengurangi saldo sistem') + ')<br>'
      + '• <b>Hapus jurnal orphan di bawah untuk memperbaiki selisih saldo.</b>'
      + '</div>';
  }
...
async function cekDuplikatTransaksi(id) {
  ...
  var targetSourceSig = getJurnalSourceSignature(target);
  ...
  if (duplikats.length === 0) {
    html += '<div class="alert alert-success" style="margin-top:12px">Tidak ditemukan transaksi yang cukup kuat untuk dianggap duplikat.</div>';
  } else {
    html += '<div style="font-weight:700;margin:12px 0 8px;color:#c62828">🔴 Ditemukan ' + duplikats.length + ' transaksi yang sangat kuat mengarah ke duplikat:</div>';
    ...
    duplikats.forEach(function(d, idx) {
      ...
      var skorLabel = d.skor >= 8 ? '🔴 Duplikat kuat' : d.skor >= 6 ? '🟠 Sangat mirip' : '🟡 Mirip';
...
async function renderBankRec() {
  ...
  var bankSummaryHtml = bankAkun.map(function(a) {
    ...
    return '<tr><td class="fw-bold">' + a.kode + '</td><td>' + a.nama + '</td>'
      ...
      + '<td>' + statusBadge + '</td>'
      + '<td class="tbl-actions"><button class="btn btn-xs btn-info" onclick="viewBankRecDetail(\'' + a.kode + '\',' + saldoSistem + ')">👁️ Detail</button>' + (savedActual && Math.abs(selisih) >= 1 ? ' <button class="btn btn-xs btn-warning" onclick="analisaKoreksiBank(\'' + a.kode + '\',' + saldoSistem + ',' + savedActual + ')">🔎 Analisa</button>' : '') + '</td></tr>';
  }).join('');
...
async function koreksiSaldoAwalBank(kode, saldoSistem, saldoAktual) {
  // Fallback terakhir jika analisa transaksi tidak menemukan penyebab spesifik.
  ...
  if (!confirm('Gunakan koreksi saldo awal sebagai langkah TERAKHIR?\n\nPastikan hasil analisa transaksi sudah direview dan tidak ada jurnal yang perlu dibetulkan terlebih dahulu.\n\nSaldo sistem: ' + fmtRp(saldoSistem) + '\nSaldo aktual: ' + fmtRp(saldoAktual) + '\nSelisih: ' + fmtRp(selisih) + '\n\nSaldo awal lama: ' + fmtRp(saLama) + '\nSaldo awal baru: ' + fmtRp(saBaru))) return;
  saldoAwalData[kode] = saBaru;
  await KDB.saveSetting('saldo_awal_' + tahun, saldoAwalData);
  showAlert('Saldo awal akun ' + kode + ' diperbarui sebagai fallback terakhir.');
  navigate('kalk-bank-rec');
}

async function getBankRecAnalysis(kode, saldoSistem, saldoAktual) {
  var allJ = await KDB.getAll('jurnal');
  var akunList = await getAkun();
  var akun = akunList.find(function(a){ return a.kode === kode; });
  var pdList = await KDB.getAll('permohonan');
  ...
  related.forEach(function(j) {
    var key = getJurnalDuplicateKey(j);
    if (!key) return;
    if (!dupMap[key]) dupMap[key] = j;
    else findings.push({ kind: 'duplicate', jurnalId: j.id, masterId: dupMap[key].id, title: 'Duplikat jurnal', severity: 'danger', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Mirip dengan jurnal ' + dupMap[key].id });
  });

  related.forEach(function(j) {
    var meta = j.meta || {};
    if (j.sumber === 'permohonan-dana' && meta.permohonanId) {
      var p = pdMap[meta.permohonanId];
      if (!p) {
        findings.push({ kind: 'orphan', jurnalId: j.id, masterId: meta.permohonanId, title: 'Jurnal yatim permohonan', severity: 'danger', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Permohonan ' + meta.permohonanId + ' sudah tidak ada' });
      } else {
        var expectedDate = p.jatuhTempo || p.tanggal || '';
        var expectedRef = p.noPOInvoice || p.id || '';
        var expectedAmount = parseFloat(p.nominal) || 0;
        ...
        var expectedKredit = p.akunKredit || '1-1100';
        var lineAkun = (j.lines || []).map(function(l) { return l.akun; });
        var mismatch = j.tanggal !== expectedDate || (j.noRef || '') !== expectedRef || Math.abs((parseFloat(j.totalDebit) || 0) - expectedAmount) > 0.01 || lineAkun.indexOf(expectedDebit) === -1 || lineAkun.indexOf(expectedKredit) === -1;
        if (mismatch) findings.push({ kind: 'stale-permohonan', jurnalId: j.id, masterId: p.id, title: 'Link permohonan tidak sinkron', severity: 'warning', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Tanggal/ref/akun/nominal belum sinkron dengan permohonan' });
      }
    }
    if (j.sumber === 'dana-masuk' && meta.danaMasukId) {
      var d = dmMap[meta.danaMasukId];
      if (!d) {
        findings.push({ kind: 'orphan', jurnalId: j.id, masterId: meta.danaMasukId, title: 'Jurnal yatim dana masuk', severity: 'danger', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Dana masuk ' + meta.danaMasukId + ' sudah tidak ada' });
      } else {
        var expectedDateDM = d.tanggal || '';
        var expectedRefDM = d.noRef || d.id || '';
        var expectedAmountDM = parseFloat(d.nominal) || 0;
        var expectedDebitDM = d.akunTerima || '1-1100';
        var expectedKreditDM = d.kategori && (d.kategori.startsWith('4-') || d.kategori.startsWith('3-') || d.kategori.startsWith('1-')) ? d.kategori : '4-2000';
        var lineAkunDM = (j.lines || []).map(function(l) { return l.akun; });
        var mismatchDM = j.tanggal !== expectedDateDM || (j.noRef || '') !== expectedRefDM || Math.abs((parseFloat(j.totalDebit) || 0) - expectedAmountDM) > 0.01 || lineAkunDM.indexOf(expectedDebitDM) === -1 || lineAkunDM.indexOf(expectedKreditDM) === -1;
        if (mismatchDM) findings.push({ kind: 'stale-danamasuk', jurnalId: j.id, masterId: d.id, title: 'Link dana masuk tidak sinkron', severity: 'warning', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Tanggal/ref/akun/nominal belum sinkron dengan dana masuk' });
      }
    }
    var targetAkun = findPreferredBankAccountCode(akunList, meta.namaBank, kode);
    if (meta.namaBank && targetAkun && targetAkun !== kode) {
      findings.push({ kind: 'wrong-bank', jurnalId: j.id, masterId: targetAkun, title: 'Salah akun bank', severity: 'warning', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Keterangan/metadata bank mengarah ke akun ' + targetAkun });
    }
    var bankLines = (j.lines || []).filter(function(l) { return l.akun === kode; });
    var hasDebit = bankLines.some(function(l) { return (parseFloat(l.debit)||0) > 0; });
    var hasKredit = bankLines.some(function(l) { return (parseFloat(l.kredit)||0) > 0; });
    if (hasDebit && hasKredit) findings.push({ kind: 'mixed-bank', jurnalId: j.id, masterId: '', title: 'Debit & kredit di akun bank sama', severity: 'warning', tanggal: j.tanggal, ket: j.keterangan, nominal: parseFloat(j.totalDebit)||0, detail: 'Perlu review detail COA karena akun bank yang sama muncul di dua sisi' });
  });
  ...
  return { akun: akun, findings: findings, related: related, pendingJurnal: pendingJurnal, selisih: saldoSistem - saldoAktual };
}

async function eksekusiTemuanBankRec(kind, jurnalId, masterId, kode, saldoSistem, saldoAktual) {
  showLoading(true);
  try {
    if (kind === 'duplicate' || kind === 'orphan') {
      await moveJurnalToTrash(jurnalId);
    } else if (kind === 'stale-permohonan') {
      var pdList = await KDB.getAll('permohonan');
      var p = pdList.find(function(x) { return x.id === masterId; });
      if (p) await syncPermohonanLinkedJurnal(p);
    } else if (kind === 'stale-danamasuk') {
    ...
    showLoading(false);
    closeModalDirect();
    setTimeout(function() { analisaKoreksiBank(kode, saldoSistem, saldoAktual); }, 250);
  } catch (err) {
    showLoading(false);
...
async function analisaKoreksiBank(kode, saldoSistem, saldoAktual) {
  ...
  try {
    ...
    function renderGroup(key, title, hint, openByDefault) {
      var items = groupedRows[key] || [];
      if (!items.length) return '';
      return '<details class="bankrec-group" ' + (openByDefault ? 'open' : '') + '>'
        + '<summary><div><div class="bankrec-group-title">' + title + '</div><div class="bankrec-group-hint">' + hint + '</div></div><span class="chip bankrec-group-count">' + items.length + ' item</span></summary>'
        + '<div class="table-wrap bankrec-group-table"><table style="font-size:0.82rem;min-width:980px"><thead><tr><th>Tipe</th><th>Tanggal</th><th>Jurnal</th><th>Keterangan</th><th>Nominal</th><th>Temuan</th><th>Aksi</th></tr></thead><tbody>' + items.join('') + '</tbody></table></div>'
        + '</details>';
    }
    var groupedHtml = renderGroup('critical', 'Temuan Kritis', 'Double entry atau jurnal yatim yang paling berpengaruh ke saldo.', true)
    ...
    var html = '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px">'
      ...
      + '<select id="bankrec-analysis-filter" style="padding:7px 10px;border:1px solid #d0d7e2;border-radius:8px" onchange="filterBankRecAnalysisRows(\'bankrec-analysis-root\', (document.getElementById(\'bankrec-analysis-search\')||{}).value || \"\", this.value)">'
      + '<option value="all">Semua Temuan (' + findings.length + ')</option>'
      ...
      + '</div>'
      + '</div>'
      + '<div class="bankrec-summary-grid">'
      ...
      + '<button class="btn btn-outline" onclick="closeModalDirect()">Tutup</button>'
      + '</div>';
    openModal(html, '🔎 Analisa Rekonsiliasi: ' + ((analisa.akun || {}).nama || kode));
  } catch (err) {
    showLoading(false);
    showAlert('Analisa rekonsiliasi gagal: ' + (err.message || err), 'danger');
  }
}
...
async function viewBankRecDetail(kode, saldoSistem) {
  ...
  related.forEach(function(j) {
    (j.lines||[]).forEach(function(l) {
      if (l.akun === kode) { totalDebit += parseFloat(l.debit)||0; totalKredit += parseFloat(l.kredit)||0; }
    });
  });
...
async function renderPermohonanDana() {
  ...
  return '<div class="page-title">📤 Permohonan Dana</div>'
    + '<div style="display:flex;gap:12px;align-items:center;margin-bottom:12px;flex-wrap:wrap">'
    + '<button class="btn btn-warning" onclick="navigate(\'monitor-forecast-bayar\')" style="display:flex;align-items:center;gap:6px;padding:10px 16px;border-radius:8px;font-weight:600;font-size:0.88rem">'
    + '📉 Lihat Forecast Pembayaran' + (overdueBayarCount > 0 ? ' <span style="background:#d32f2f;color:white;border-radius:50%;padding:2px 7px;font-size:0.75rem;margin-left:4px">' + overdueBayarCount + ' overdue</span>' : '')
    + '</button></div>'
    + pendingBanner
...
async function renderDanaMasuk() {
  ...
  return '<div class="page-title">📥 Dana Masuk</div>'
    + '<div style="display:flex;gap:12px;align-items:center;margin-bottom:12px;flex-wrap:wrap">'
    + '<button class="btn btn-info" onclick="navigate(\'monitor-forecast-terima\')" style="display:flex;align-items:center;gap:6px;padding:10px 16px;border-radius:8px;font-weight:600;font-size:0.88rem">'
    + '📈 Lihat Forecast Penerimaan' + (overdueTerimaCount > 0 ? ' <span style="background:#d32f2f;color:white;border-radius:50%;padding:2px 7px;font-size:0.75rem;margin-left:4px">' + overdueTerimaCount + ' overdue</span>' : '')
    + '</button></div>'
    + pendingBanner
...
async function runAIAnalysis() {
  ...
  setTimeout(async function() {
    ...
    // 14. Unusual Amounts (Outlier Detection)
      ...
      try {
    function normalizeOutlierPattern(text) {
      return normalizeCompareText(text)
        .replace(/\b(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agu|agt|sep|okt|nov|des)\b/g, '')
        .replace(/\b20\d{2}\b/g, '')
        .replace(/\d+/g, '')
        .replace(/\s+/g, ' ')
        .trim();
    ...
    allJurnal.forEach(function(j) {
      ...
      if (j.tipe === 'penutup') return;
      var amount = Math.abs(parseFloat(j.totalDebit) || 0);
      if (amount <= 0) return;
      var patternKey = normalizeOutlierPattern(j.keterangan || '') || normalizeOutlierPattern(j.noRef || j.id);
      allAmounts.push({
        val: amount,
        ref: j.noRef || j.id,
        tanggal: j.tanggal,
      ...
      if (threshold > 0) {
        ...
        allAmounts.forEach(function(a) {
          if (a.val > threshold) {
            ...
            if (outlierExcluded.indexOf(outlierKey) !== -1) return;
            // Jika pola transaksi berulang (contoh: payroll bulanan), jangan ditandai anomali
            var similarRecurring = allAmounts.filter(function(b) {
              if (b.jurnalId === a.jurnalId) return false;
              if (!a.patternKey || !b.patternKey || a.patternKey !== b.patternKey) return false;
              var tolerance = Math.max(10000, a.val * 0.1); // toleransi 10%
              return Math.abs(b.val - a.val) <= tolerance;
            }).length;
            if (similarRecurring >= 2) {
              recurringCount++;
              return;
            }
            var ratioMean = meanVal > 0 ? (a.val / meanVal) : 0;
            var reason = 'Nominal ' + ratioMean.toFixed(1) + 'x di atas rata-rata ' + fmtRp(meanVal) + ' (ambang outlier ' + fmtRp(threshold) + ')';
            if (a.source) reason += ' | Sumber: ' + a.source;
            outliers.push(Object.assign({}, a, { reason: reason }));
          }
        });
        if (recurringCount > 0) info.push('ℹ️ ' + recurringCount + ' transaksi nominal besar tidak ditandai anomali karena terdeteksi pola berulang (contoh payroll/periode rutin).');
        outliers.sort(function(a,b) { return b.val - a.val; });
        outliers = outliers.slice(0,10);
      }
    }
    if (outliers.length > 0) {
      warnings.push({ severity: 'warning', title: '⚠️ Transaksi Tidak Lazim / Outlier (' + outliers.length + ' ditemukan)', actionButtons: '<button class="btn btn-info btn-sm" onclick="navigate(\'jurnal-umum\')">📓 Buka Jurnal Umum</button> <button class="btn btn-success btn-sm" onclick="abaikanSemuaOutlier()">✅ Abaikan Semua</button>', detail: outliers.map(function(o) { var oKey = (o.tanggal||'') + '_' + (o.ref||'') + '_' + o.val; return '<tr><td>' + fmtDate(o.tanggal) + '</td><td>' + o.ref + '</td><td>' + (o.ket||'-') + '</td><td class="text-red fw-bold">' + fmtRp(o.val) + '</td><td style="font-size:0.78rem;line-height:1.35">' + (o.reason||'-') + '</td><td class="tbl-actions"><button class="btn btn-xs btn-info" onclick="reviewOutlierDetail(\'' + o.jurnalId + '\')">👁️ Review</button> <button class="btn btn-xs btn-success" onclick="abaikanOutlier(\'' + oKey.replace(/'/g,'') + '\')">✅ Benar</button></td></tr>'; }).join('') + '<tr><td colspan="6" style="font-size:0.82rem;color:#666;padding-top:8px"><b>Rekomendasi:</b> Koreksi hanya jika transaksi memang salah input. Jika valid, klik "✅ Benar" agar tidak ditandai lagi.</td></tr>', isTable: true, headers: '<th>Tanggal</th><th>Ref</th><th>Keterangan</th><th>Jumlah</th><th>Alasan Ditandai</th><th>Aksi</th>', group: 'anomali' });
    } else {
